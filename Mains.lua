    ...
    if state then
        ...
        if Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if moveDir.Magnitude > 0 then
                -- Inject velocity tambahan
                local currentVelocity = HRP.Velocity
                local additionalVelocity = moveDir * SpeedValue
            
            -- Terapkan velocity baru (hanya horizontal)
            local newVelocity = Vector3.new(
                additionalVelocity.X,
                currentVelocity.Y,  -- Pertahankan velocity Y (gravitasi/jump)
                additionalVelocity.Z
            )
            
            -- Gunakan BodyVelocity yang di-destroy setiap frame (lebih stealth)
            local bv = Instance.new("BodyVelocity")
            bv.Velocity = newVelocity
            bv.MaxForce = Vector3.new(4000, 0, 4000)  -- Terbatas, lebih natural
            bv.P = 1000
            bv.Parent = HRP
            
            game:GetService("Debris"):AddItem(bv, 0.1) -- Auto destroy
            Notify("Success Applied","check","Run Speed Activated")
        end
    end
    else
        RunService:UnbindFromRenderStep("VelocitySpeed")
        Notify("Success Applied","check","Run Speed Deactivated")
    end
end
